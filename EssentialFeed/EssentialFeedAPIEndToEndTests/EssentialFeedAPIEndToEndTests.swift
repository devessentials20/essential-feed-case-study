...
//
//  Created by panditpakhurde on 25/04/20.
...
class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedResult_matcheFixedTestAccountData() {
        ...
        case let .success(imageFeed)?:
            XCTAssertEqual(imageFeed.count, 8, "Expected 8 images in the test account image")
    ...
    private func getFeedResult(file: StaticString = #file, line: UInt = #line) -> FeedLoader.Result? {
        ...
        let client = URLSessionHTTPClient(URLSession(configuration: .ephemeral))
        let loader = RemoteFeedLoader(url: testServerURL, client: client)
        trackForMemoryLeaks(client, file: file, line: line)
        trackForMemoryLeaks(loader, file: file, line: line)
        let exp = expectation(description: "Wait for load completion")
        var receivedResuls: FeedLoader.Result?
        loader.load() { result in
            receivedResuls = result
            exp.fulfill()
        }
        
        wait(for: [exp], timeout: 8.0)
        return receivedResuls
    }
