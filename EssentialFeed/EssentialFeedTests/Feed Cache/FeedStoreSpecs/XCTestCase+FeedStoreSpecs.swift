...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func assertThatDeleteEmptiedPreviousInsertedCache(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        insert((feed: uniqueImageFeed().local, timestamp: Date()), to: sut)
    ...
    func assertThatStoreSideEffectsRunsSerially(on sut: FeedStore, file: StaticString = #file, line: UInt = #line) {
        ...
        XCTAssertEqual(completedOperationsInOrder, [op1, op2, op3], "Expected side-effect to run serially, but operations finished in the wrong order", file: file, line: line)
    }
